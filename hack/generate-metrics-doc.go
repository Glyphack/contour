...
import (
	...
	clinet_model "github.com/prometheus/client_model/go"
)

// Collect all the label names for this metric and return them as
// a comma-deparated string.
func labels(mf *clinet_model.MetricFamily) string {
	var l []string
...
func typeof(mf *clinet_model.MetricFamily) string {
	switch t := mf.GetType(); t {
	case clinet_model.MetricType_COUNTER, clinet_model.MetricType_GAUGE,
	clinet_model.MetricType_SUMMARY, clinet_model.MetricType_HISTOGRAM:
		return fmt.Sprintf(
...
func runPromlint(family []*clinet_model.MetricFamily) {
